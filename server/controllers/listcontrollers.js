...
console.log(user._id)
    ...
    return res.status(200).json({ success: true, message: "User added to list", list });
  } catch (error) {
...
console.log("listid",listid)
    ...
    if (!list.users.length) {
      return res.status(404).json({ success: false, message: "No users found in this list" });
    }
...
exports.deleteUserFromList = async (req, res) => {
  try {
    ...
    return res.status(200).json({ success: true, message: "User removed from list", list });
  } catch (error) {
    return res.status(500).json({ success: false, error: error.message });
  }
};
